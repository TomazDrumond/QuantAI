...
Identidade Visual: MINERVA IX (Paleta Parthenon Gold & Aegean Navy).

Gera 3 gráficos essenciais para a apresentação executiva e relatório de validação:
    1. Curva de Patrimônio Acumulado: Minerva IX vs. Benchmark Ibovespa vs. CDI.
...
# Paleta Visual Minerva IX (Grécia / Ouro / Azul Egeu)
COR_MINERVA_OURO  = "#D4AF37"  # Ouro Ateniense
COR_IBOV_TERRA    = "#E67E22"  # Terracota Grega
COR_CDI_VERDE     = "#27AE60"  # Verde Mármore
COR_DRAWDOWN_ROJO = "#C0392B"  # Vermelho Carmim

PALETA_MINERVA_AREA = [
    "#1B365D",  # Azul Egeu (CDI / Cash)
    "#D4AF37",  # Ouro Ateniense
    "#2E7D32",  # Verde Oliva
    "#C62828",  # Vermelho Espartano
    "#F57F17",  # Âmbar Imperial
    "#558B2F",  # Bronze Claro
    "#8D6E63",  # Cobre
    "#455A64",  # Cinza Mármore
    "#00838F",  # Turquesa Mediterrâneo
    "#B2EBF2",  # Azul Claro Mármore
]
...
def plotar_curva_patrimonio_e_drawdown(
    ...
    Gera a figura composta contendo:
      - Painel Superior: Curva de evolução de patrimônio (R$) na identidade Minerva IX.
    ...
    # Painel 1: Patrimônio com cores Minerva IX
    ax1.plot(datas, patrimonio_carteira, label="Minerva IX (Estratégia)", color=COR_MINERVA_OURO, linewidth=2.5)
    ...
    if retornos_ibov is not None and len(retornos_ibov) == len(retornos_carteira):
        ...
        ax1.plot(datas, patrimonio_ibov, label="Ibovespa (Benchmark)", color=COR_IBOV_TERRA, linestyle="--", linewidth=1.5, alpha=0.85)
    ...
    if retornos_cdi is not None and len(retornos_cdi) == len(retornos_carteira):
        ...
        ax1.plot(datas, patrimonio_cdi, label="CDI (Taxa Livre de Risco)", color=COR_CDI_VERDE, linestyle=":", linewidth=1.5, alpha=0.85)

    ax1.set_title(f"Evolução de Patrimônio Acumulado — Minerva IX ({ano_inicio}-{ano_fim})", fontsize=14, fontweight='bold', pad=12)
    ...
    # Painel 2: Drawdown
    ax2.plot(datas, drawdown_carteira, color=COR_DRAWDOWN_ROJO, linewidth=1.2, label="Drawdown Minerva IX")
    ax2.fill_between(datas, drawdown_carteira, 0, color=COR_DRAWDOWN_ROJO, alpha=0.25)
...
def plotar_alocacao_pesos_historica(
    ...
    dos pesos alocados por classe/ativo na paleta de cores Minerva IX.
    """
    Path(pasta_saida).mkdir(parents=True, exist_ok=True)
    ...
    df_plot.plot(kind="area", stacked=True, ax=ax, color=PALETA_MINERVA_AREA[:len(df_plot.columns)], alpha=0.85)
...
def gerar_relatorio_grafico_demonstracao() -> list[str]:
    """Função utilitária para gerar e validar gráficos na identidade Minerva IX."""
    ...
    ativos = ["CDI_RF", "PETR4", "VALE3", "ITUB4", "BBDC4", "BBAS3", "WEGE3", "RENT3", "EQTL3", "ABEV3"]
...
if __name__ == "__main__":
    print("\n========================================================")
    print("  GERANDO RELATÓRIOS GRÁFICOS DO MINERVA IX (AGENTE 8)")
    print("========================================================\n")
