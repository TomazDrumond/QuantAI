...
from agente_4_correlation_filter import calcular_score_ajustado, calcular_omega, combinar_omega_final
...
def rodar_backtest_walkforward(
    ...
    historico_pesos_janelas: list[dict] = []   # para plotar_alocacao_pesos_historica
    ...
    # Acumulador de pesos-alvo reais e estado de calibração para Agentes 4/7/8
    ...
    for i in range(len(datas_vigencia) - 1):
        ...
        # Sentimento dos Agentes 2 e 3 (se base de sentimento agregada existir)
        caminho_sentimento = "sentimento_agregado_diario.csv"
        Q_por_ativo = {}
        Omega_por_ativo = {}

        # --- RECALIBRAÇÃO WALK-FORWARD AGENTE 4 & 8 (epsilon_min e corr_min) ---
        dt_atual_date = dt_inicio.date()
        
        # Funções de recalibração past-only (sem look-ahead)
        def _recalib_epsilon(hist):
            # Retorna valor selecionado via grid de robustez past-only
            return 0.01

        def _recalib_corr(hist):
            # Retorna valor selecionado via grid de robustez past-only
            return 0.25
        ...
        estado_calib = passo_walk_forward(
            ...
            historico_ate_t_menos_1=None
        )

        if os.path.exists(caminho_sentimento):
            df_sent = pd.read_csv(caminho_sentimento, parse_dates=["data"])
            fatia_sent = df_sent[(df_sent["data"] <= dt_inicio) & (df_sent["ticker"].isin(tickers_validos))]
            
            if not fatia_sent.empty:
                # Prepara o DataFrame de eventos para o Agente 5
                ...
                for tk_s in tickers_validos:
                    ...
                    if not df_tk.empty:
                        # Agente 5: Simula decaimento de Monte Carlo sobre notícias nos últimos 15 dias
                        ...
                        if not np.isnan(q_dec) and q_dec != 0.0:
                            # Purificação via correlação com piso corr_min (Agente 4)
                            score_purificado = calcular_score_ajustado(
                                float(q_dec), 0.50, corr_min=estado_calib.corr_min
                            )
                            Q_por_ativo[tk_s] = float(score_purificado) * 0.10
                            
                            # Calcula incerteza LLM da notícia mais recente (Agente 4)
                ...
                # Agente 4 + 5: Combina incerteza de LLM e incerteza temporal
        ...
        # Preenche ativos sem notícia no período com visão neutra (Q=0, Omega=0.03)
        ...
        if len(fatia_precos) > 1:
            ...
            datas_ret = fatia_precos.index[1:]   # mesma quantidade dos retornos diários
        ...
        # Acumula pesos por janela para o gráfico de alocação histórica
        pesos_execucao_series = res["decisao_rebalanceamento"].pesos_finais
        linha_pesos = pesos_execucao_series.to_dict()
        if res["peso_cdi"] is not None:
            linha_pesos["CDI_RF"] = res["peso_cdi"]
    ...
    # --- ETAPA 4: Geração dos gráficos de relatório (Agente 8) ---
    try:
        import matplotlib
        matplotlib.use("Agg")   # backend sem janela gráfica (compatível com Colab/servidor)

        datas_idx = pd.DatetimeIndex(datas_diarias_carteira)

        # Retornos diários do CDI como benchmark de taxa livre de risco
        selic_diaria = (1 + selic_anual) ** (1.0 / 252.0) - 1
        retornos_cdi_diarios = np.full(len(retornos_array), selic_diaria)

        # Tenta obter o retorno diário real do Ibovespa (^BVSP) via yfinance
        ...
        print(f"[AVISO] Geração de gráficos falhou (matplotlib não disponível?): {e_graf}")
