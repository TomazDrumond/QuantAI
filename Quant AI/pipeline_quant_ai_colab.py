...
def rodar_backtest_walkforward(
    ...
    for i in range(len(datas_vigencia) - 1):
        ...
        # Item E: Covariância encolhida via Ledoit-Wolf (Fase 0)
        if _LEDOIT_WOLF_DISPONIVEL:
            try:
                Sigma = LedoitWolf().fit(retornos_hist.values).covariance_
            except Exception:
                Sigma = retornos_hist.cov().values + np.eye(len(tickers_validos)) * 1e-4
        else:
            Sigma = retornos_hist.cov().values + np.eye(len(tickers_validos)) * 1e-4

        # Item D: Prior de equilíbrio Pi por Otimização Reversa (Π = δΣw_mkt)
        ...
        if os.path.exists(caminho_sentimento):
            ...
            if "data" in df_s_full.columns:
                ...
                if not hist_sent_acumulado.empty and "retorno_realizado_fwd" not in hist_sent_acumulado.columns:
                    # Preenche retorno realizado fwd com dados históricos de preços
                    rets_fwd = []
        ...
        if os.path.exists(caminho_sentimento):
            ...
            if not fatia_sent.empty:
                # Item A: Validação anti-look-ahead via Agente 1
                ...
                for tk_s in tickers_validos:
                    ...
                    if not df_tk.empty:
                        ...
                        if not np.isnan(q_dec) and q_dec != 0.0:
                            # Item C: Filtro de correlação real móvel (Agente 4)
                            ...
                            # Item J: Escala alinhada (0.05 default)
                            Q_por_ativo[tk_s] = float(score_purificado) * escala_q
        ...
        r_cdi_quadri = (1 + selic_anual_janela) ** (dias_uteis_janela / 252.0) - 1
    ...
    try:
        ...
        selic_media_diaria = (1 + 0.10) ** (1.0 / 252.0) - 1
        retornos_cdi_diarios = np.full(len(retornos_array), selic_media_diaria)
