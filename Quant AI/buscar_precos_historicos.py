...
def buscar_em_lotes(
    ...
    INDIVIDUALMENTE numa segunda passada -- boa parte das falhas de
    "possibly delisted" em lote são instabilidade transitória da API,
    não delistagem real, e costumam se resolver numa tentativa isolada.
    ...
    if falhas_em_lote:
        print(f"\n[RETENTATIVA] {len(falhas_em_lote)} ticker(s) falharam em lote -- "
              f"tentando individualmente (isola instabilidade de API de delistagem real): {falhas_em_lote}")
        recuperados = []
        ...
        if recuperados:
            print(f"[RECUPERADOS na retentativa individual, provável instabilidade de API, não delistagem]: {recuperados}")
        if falhas_finais:
            print(f"[FALHA CONFIRMADA -- provável mudança de código/delistagem real, precisa de mapeamento manual]: {falhas_finais}")
