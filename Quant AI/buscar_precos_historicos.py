...
def buscar_preco_individual_via_ticker(ticker: str, inicio: str, fim: str) -> pd.DataFrame:
    """
    Caminho alternativo, deliberadamente DIFERENTE de buscar_precos_b3:
    usa yf.Ticker(...).history() em vez de yf.download(). O download()
    multi-ticker usa threads internamente por padrão, e há um problema
    documentado no repositório do yfinance onde a busca do cookie/crumb
    de autenticação falha de forma consistente (não aleatória) para um
    subconjunto de tickers sob esse caminho -- inclusive quando chamado
    com lista de 1 elemento, porque ainda é o MESMO código internamente.
    Ticker().history() não usa essa via multithreaded, e costuma
    recuperar exatamente os casos que download() falha de forma
    consistente (não é sobre delistagem real).
    """
    import yfinance as yf
    t = yf.Ticker(f"{ticker}.SA")
    hist = t.history(start=inicio, end=fim, auto_adjust=True)
    if hist.empty:
        return pd.DataFrame(columns=["data", "ticker", "close"])
    hist = hist.reset_index()[["Date", "Close"]].rename(columns={"Date": "data", "Close": "close"})
    hist["ticker"] = ticker
    hist["data"] = pd.to_datetime(hist["data"]).dt.tz_localize(None)
    return hist[["data", "ticker", "close"]]
...
def buscar_em_lotes(
    ...
    if falhas_em_lote:
        print(f"\n[RETENTATIVA] {len(falhas_em_lote)} ticker(s) falharam em lote -- "
              f"tentando via yf.Ticker().history() (caminho diferente de yf.download(), "
              f"evita o bug conhecido de crumb/cookie sob multithreading): {falhas_em_lote}")
        recuperados = []
        for ticker in falhas_em_lote:
            try:
                bloco_individual = buscar_preco_individual_via_ticker(ticker, inicio, fim)
        ...
        if falhas_finais:
            ...
            if len(falhas_finais) == len(falhas_em_lote):
                print(
                    "[ALERTA] TODAS as retentativas falharam de forma idêntica MESMO usando "
                    "yf.Ticker().history() (caminho diferente do download() multithreaded) -- "
                    "isso descarta o bug de crumb/cookie sob threading como causa única. Duas "
                    "hipóteses restantes: (1) bloqueio de IP da Yahoo especificamente sobre a "
                    "faixa de IPs do Colab (comum, por uso compartilhado/abusivo de muitos "
                    "usuários simultâneos) -- tente reiniciar o runtime do Colab por completo "
                    "(Ambiente de execução > Desconectar e excluir o runtime, não só reiniciar "
                    "sessão) para tentar obter um IP de saída diferente; (2) parte real destes "
                    "tickers pode de fato ter mudado de código/se fundido (ex: BTOW3+LAME4-> "
                    "AMER3 em 2021, VIVT4->VIVT3, VVAR3->VIIA3->depois outro rename) -- "
                    "esses vão continuar falhando mesmo com IP novo, e precisam de mapeamento manual."
                )
