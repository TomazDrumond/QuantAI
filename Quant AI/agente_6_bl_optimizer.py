...
def verificar_calibracao_tau_omega(
    Sigma: np.ndarray,
    tau: float,
    Omega_valores: list[float],
    razao_maxima_aceitavel: float = 5.0,
) -> None:
    """
    Checagem de sanidade que deveria rodar SEMPRE antes de aceitar um
    resultado do Agente 6 como válido. Descoberta na integração real
    (teste de fumaça no Colab, out/2026): com tau e Sigma escolhidos sem
    calibrar a escala contra Omega, a precisão do prior (tau*Sigma)^-1
    pode ficar 40-75x maior que a precisão da view (1/Omega) -- nesse
    caso a view é estatisticamente afogada pelo prior, e TODOS os ativos
    colapsam para perto do prior, independente do sentimento. Sintoma
    visível: pesos muito parecidos entre ativos, mesmo com views opostas.

    Levanta um aviso (não exceção -- pode ser intencional em alguns
    casos) se a razão entre as precisões ultrapassar razao_maxima_aceitavel.
    """
    precisao_prior = np.diag(np.linalg.inv(tau * Sigma))
    precisao_prior_media = float(np.mean(precisao_prior))
    precisao_views_media = float(np.mean([1.0 / o for o in Omega_valores if o > 0]))

    razao = precisao_prior_media / precisao_views_media if precisao_views_media > 0 else np.inf
    if razao > razao_maxima_aceitavel:
        print(
...
def calcular_posterior_bl(
    ...
    if len(tickers_com_view) > 0:
        verificar_calibracao_tau_omega(Sigma, tau, list(Omega_diag))
...
def bl_optimizer(
    ...
    w = cp.Variable(n)
    risco = cp.quad_form(w, cp.psd_wrap(Sigma_ext))
