...
def rodar_pipeline_sintetico(seed: int = 0) -> None:
    ...
    resultado = rodar_um_rebalanceamento(
        ...
        valor_carteira=100_000, w_max=0.20, r_cdi=0.04,
    )

    print("=== Teste de fumaça (dados sintéticos) ===")
    print("Pesos-alvo (Agente 6):", resultado["pesos_alvo_bl"].round(4).to_dict())
    print("Ativos executados (Agente 7):", resultado["decisao_rebalanceamento"].ativos_executados)
    print("Turnover:", round(resultado["decisao_rebalanceamento"].turnover, 4))
    print("Custo total de transação:", round(resultado["custos"].custo_total, 2))
